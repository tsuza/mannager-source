    ...
    Renderer: text::Renderer,
{
    ...
    /// Sets the delay between when the [`Tooltip`] is hovered over and when its content is shown
    ...
    Renderer: text::Renderer,
{
    ...
    fn update(
        ...
        match (&state, cursor_position) {
            ...
            (State::Hovered { at, .. }, Some(cursor_position)) if at.elapsed() < self.delay => {
    ...
    fn overlay<'b>(
        ...
        let tooltip = match *state {
            State::Hovered {
                cursor_position,
                at,
            } if at.elapsed() > self.delay => Some(overlay::Element::new(Box::new(Overlay {
